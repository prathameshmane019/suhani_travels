...
const homeDataCache = new node_cache_1.default({ stdTTL: 3600 }); // Cache for 1 hour
...
router.get("/home-data", async (req, res) => {
    try {
        ...
        const popularRoutes = await trip_1.Trip.aggregate([
            {
                $group: {
                    _id: "$route",
                    tripCount: { $sum: 1 },
        ...
        const featuredBuses = await bus_1.BusModel.find({ status: 'active' })
            .sort({ rating: -1 })
            .limit(4)
            .select('model type amenities image rating');
        // Fetch statistics
        const totalUsers = await user_1.UserModel.countDocuments();
        const totalBuses = await bus_1.BusModel.countDocuments({ status: 'active' });
        const totalTrips = await trip_1.Trip.countDocuments(); // Consider only future trips for a more accurate "passengers served"
        // For "passengers served", we can sum up booked seats from past trips
        const passengersServedResult = await trip_1.Trip.aggregate([
            {
                $match: {
        ...
        const citiesCoveredResult = await bus_1.RouteModel.aggregate([
            { $unwind: "$stops" },
            {
                $group: {
                    _id: null,
