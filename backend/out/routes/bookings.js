...
const getBookedSeats = async (tripId) => {
    try {
        const bookings = await booking_1.Booking.find({ tripId, status: 'confirmed' }).select('seatNumbers');
...
router.get('/by-mobile', async (req, res) => {
    try {
        ...
        const [bookings, total] = await Promise.all([
            booking_1.Booking.find({ 'passengerDetails.phone': mobile })
                .populate({
                path: 'tripId',
                select: 'route bus date price schedule',
                populate: [
                ...
                .skip(skip)
                .limit(Number(limit)),
            booking_1.Booking.countDocuments({ 'passengerDetails.phone': mobile })
        ]);
        res.json({
            bookings,
...
router.post('/', async (req, res) => {
    try {
        const { tripId, userId, seatNumbers, passengerDetails, totalPrice, boardingPoint, dropoffPoint } = req.body;
        ...
        const trip = await trip_1.Trip.findById(tripId).populate('bus').populate('route');
        if (!trip) {
            throw new Error('Trip not found');
        }
        // Validate boarding and dropoff points against the trip's route stops
        const routeStops = trip.route.stops;
        const foundBoardingPoint = routeStops.find((s) => s.name === boardingPoint.name);
        const foundDropoffPoint = routeStops.find((s) => s.name === dropoffPoint.name);
        ...
        if (maxSeat > trip?.bus?.seats) {
            throw new Error(`Invalid seat number. Maximum seat number is ${trip?.bus?.seats}`);
        }
        ...
        const populatedBooking = await booking_1.Booking.findById(newBooking._id)
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        const booking = await booking_1.Booking.findByIdAndUpdate(id, { status, updatedAt: new Date() }, { new: true }).populate('tripId', 'route bus date schedule price');
        if (!booking) {
            return res.status(404).json({ message: 'Booking not found' });
        }
...
router.patch('/:id/cancel', async (req, res) => {
    try {
        ...
        const booking = await booking_1.Booking.findById(id).populate('tripId', 'date');
        if (!booking) {
            return res.status(404).json({ message: 'Booking not found' });
        }
        ...
        const tripDate = new Date(booking.tripId.date);
        const now = new Date();
...
router.get('/stats/overview', async (req, res) => {
    try {
        ...
        const confirmedBookings = await booking_1.Booking.countDocuments({ status: 'confirmed' });
        const cancelledBookings = await booking_1.Booking.countDocuments({ status: 'cancelled' });
        const pendingBookings = await booking_1.Booking.countDocuments({ status: 'pending' });
        // Calculate total revenue from confirmed bookings
        const revenueResult = await booking_1.Booking.aggregate([
...
router.get('/availability/:tripId', async (req, res) => {
    try {
        ...
        const trip = await trip_1.Trip.findById(tripId).populate('bus');
        if (!trip) {
        ...
        res.json({
            ...
            bookedSeats,
            availableSeats: (trip.bus?.seats || 0) - bookedSeats.length,
        });
    }
    catch (error) {
